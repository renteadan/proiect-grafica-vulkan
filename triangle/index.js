...
function createVertexBuffer(buffer, bufferMemory, byteLength) {
  ...
  let verticesBuffer = ArrayBuffer.fromAddress(dataPtr.$, bufferInfo.size);
...
let vertSrc = GLSL.toSPIRVSync({
  source: fs.readFileSync(`./triangle/shaders/triangle.vert`),
  extension: `vert`
}).output;

let fragSrc = GLSL.toSPIRVSync({
  source: fs.readFileSync(`./triangle/shaders/triangle.frag`),
  extension: `frag`
}).output;
