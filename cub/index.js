...
const {copyBuffer, createBuffer,uploadBufferData,getMemoryTypeIndex , memoryCopy } = require('./buffer')
...
let vertSrc = GLSL.toSPIRVSync({
  source: fs.readFileSync(`./rotation/shaders/cube.vert`),
  extension: `vert`
}).output;

let fragSrc = GLSL.toSPIRVSync({
  source: fs.readFileSync(`./rotation/shaders/cube.frag`),
  extension: `frag`
}).output;
...
let grassImage = new Texture2D().fromImagePath("./rotation/assets/grass-block.png");
